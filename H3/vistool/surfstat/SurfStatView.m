function [ a, cb ] = SurfStatView( struct, surf, title, background, isflat)

%Viewer for surface data or P-value or Q-value structures.
% 
% Usage: [ a, cb ] = SurfStatView( struct, surf [,title [,background [,isflat] ] ] );
% 
...
% isflat        = if use flat option for shading feature of the surfview.
%
...
%
% Modified by Xi-Nian Zuo in Jan 09, 2015 at IPCAS.
...
if ~isstruct(struct)
    if strcmp(isflat, 'true')
        [a,cb]=SurfStatViewDataFlat(struct,surf,title,background);
    else
    ...
    if isfield(struct,'P')
        ...
        if strcmp(isflat, 'true')
            [a,cb]=SurfStatViewDataFlat(tt,surf,title,background);
        else
