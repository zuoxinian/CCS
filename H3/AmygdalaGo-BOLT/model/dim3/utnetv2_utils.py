...
class BidirectionAttention(nn.Module):
    def __init__(self, feat_dim, map_dim, out_dim, heads=4, dim_head=64, attn_drop=0., proj_drop=0.,
        ...
        self.high_v = SemanticMapFusion(feat_dim)
        self.low_v = SemanticMapFusion(map_dim)
    ...
    def forward(self, feat, semantic_map):
        ...
        res_high_v = self.high_v(feat_v)
        res_low_v = self.low_v(map_v)
        ...
        return feat_out + res_high_v, map_out +res_low_v
...
class SemanticMapFusion(nn.Module):
    def __init__(self, dim, heads=4, depth=1, norm=nn.BatchNorm3d, attn_drop=0., proj_drop=0.):
        super().__init__()
    ...
    def forward(self, map_list):
        B, _, D, H, W = map_list.shape
        proj_maps = (map_list).view(B, self.dim, -1).permute(0, 2, 1)
        #B, L, C where L=DHW

        attned_maps = self.fusion(proj_maps)


        maps_out = attned_maps.permute(0, 2, 1).view(B, self.dim, D, H, W)
