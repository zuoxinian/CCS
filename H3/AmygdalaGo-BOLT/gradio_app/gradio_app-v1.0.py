...
class MiddleLayer:
    ...
    def merge_xy(this):
        minlay.select_flag = False
        xy_old = this.get_xy()
        xy_new = this.get_xy_test()
        
        
        xy_merge_list = []
        for i in range(0, len(xy_old)):
            if np.sum(xy_new[int(i)]) > 0:
                
                temp = np.rot90((np.concatenate((merge_img_seg_v2(xy_old[int(i)], xy_new[int(i)])))))
...
def main():
    ...
    with gr.Blocks(title=title_web_page, equal_height = True) as app:
            ...
            with gr.Row(equal_height = False):
                ...
                with gr.Column():
                    nii_selection = gr.Dropdown(["demo_001.nii", "demo_003.nii", "demo_004.nii", "demo_005.nii", "demo_006.nii"], show_label = False, height = 80, info="Select demo file")
                    def nii_selection_function(obj):
                        minlay.select_flag = True
                        if obj == "demo_001.nii":
                            minlay.read_nii_select("/root/autodl-tmp/v1.1/gradio_app/input/demo_001.nii")
