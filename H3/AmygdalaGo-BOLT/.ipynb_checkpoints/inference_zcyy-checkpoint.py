...
def pipline(net, data, args, app=True):
    net.eval()
    inference = get_inference(args)
    with torch.no_grad():
        if app:
            images, img_origin = data
        else:
        ...
        if app: 
            ...
            save_path = os.path.join(args.output_path, name)  # acdc_3d_utnetv2, acdc_3d_unet
...
def get_parser():
    ...
    parser.add_argument('--output_path', type=str, default=r'../app_demo_data/outputs/', help='dataset name')
...
def main():
    ...
    net.cuda()
    img = load_data(args.inputdata, False)

#     pipline(net, img, args, False)
    
def main_array(input_array):
    
    print("input_array type is: {}".format(type(input_array)))
    ...
    net, _ = init_network(args)
    net.cuda()
    img = load_data(input_array)

    output = pipline(net, img, args)
