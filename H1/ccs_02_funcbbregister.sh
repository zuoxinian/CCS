...
then
	...
	if [ ! -e ${func_mask_dir}/brain.mni305.2mm.nii.gz ]; then
		...
		do
			...
			mri_surf2surf --srcsubject fsaverage --sval brain.fsaverage.${hemi}.nii.gz --hemi ${hemi} --cortex --trgsubject ${fsaverage} --tval brain.${fsaverage}.${hemi}.nii.gz --surfreg sphere.reg
