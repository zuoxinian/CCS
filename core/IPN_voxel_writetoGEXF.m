function IPN_voxel_writetoGEXF(CIJ, fname, labels, arcs)
...
fprintf(fid, '  <edges> \r');
for k = 1:Nedge
    edl = ['   <edge id="' num2str(k-1) '" source="' num2str(edge_list(k,1)-1) '" target="' ...
                num2str(edge_list(k,2)-1) '" weight="' num2str(edge_list(k,3)) '" />'];
    fprintf(fid, '%s \r', edl);
end
%% CLOSE
fprintf(fid, '  </edges> \r');
