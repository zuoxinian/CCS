function   [ec, sc] = ccshcp_core_scec(CIJ, type)
% Computes the subgraph and eigenvector centralities of the network
% represented by the adjacency matrix CIJ. It returns a vector of subgraph
% or eigenvector centralities for each node of the network (as they are ordered in the
% adjacency matrix).
%
% INPUTS:
...
if n < 1000
    ...
    sc = double(reshape(sc, length(sc), 1));
    %EC
    ec = abs(V(:,1));
